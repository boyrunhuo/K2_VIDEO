        ...
        success: function (res) {
            ...
            for (var i = 0; i < res.videoList.length; i++) {
                ...
                $("#fileList").append(
                    ...
                    + '<td>' + res.videoList[i].videoMD5 + '</td>'
                    ...
                    + '<td>' + '<button class="btn btn-default belongbtn" id="">查看视频归属</button>' + '</td>'
                    + '</tr>'
                )
            }
            ...
            $(".labelbtn").click(function () {
                $(".labelbtn").attr("disabled", true);
                var labelmd5 = $(this).parent().prev().prev().prev().prev().prev().text();//获得文件MD5
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        $('#labP').html("");
                        if (res.labelListUnique.length == 0) {
                            $("#labP").append("暂无标签");
                        } else {
            ...
            $(".belongbtn").click(function () {
                  var videoMD5=$(this).parent().prev().prev().prev().prev().prev().prev().text();
                showVideoBe(videoMD5);
                $("#videoB").show();

            })

            $("#videoBClose").click(function () {
                $("#videoB").hide();
            })


            function showVideoBe(videoMD5) {
                $('#videoBe').html("");
               ...
               $.ajax({
                    ...
                    success: function (res) {
                        var share;
                        console.log("获取视频归属成功")
                        if(res.FileInf.length==0){
                                 $("#videoBe").append(
                                    '<tr>'
                                    + '<td>' + "暂无数据" + '</td>'
                                    + '<td>'  + '</td>'
                            ...
                            for (var i = 0; i < res.FileInf.length; i++) {
                                if (res.FileInf[i].videoState == '0') {
                                    share = "未共享";
                                } else {
                                    share = "已共享";
                                }
                                var videoName = res.FileInf[i].videoName.substring(res.FileInf[i].videoName.lastIndexOf('/') + 1);
                                $("#videoBe").append(
                                    '<tr>'
                                    + '<td>' + res.FileInf[i].userName + '</td>'
                                    + '<td>' + res.FileInf[i].videoName + '</td>'
                                    + '<td>' + share + '</td>'
